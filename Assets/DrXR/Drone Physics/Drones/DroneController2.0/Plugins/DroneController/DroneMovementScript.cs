    ...
    namespace Physics
    {
		...
		public class DroneMovementScript : MonoBehaviour
		{
			...
			private void OnDrawGizmos()
			{
				if (Application.isEditor && Application.isPlaying) { 
					...
					if (droneRigidBody)
					{
						...
						Gizmos.DrawRay(droneRigidBody.transform.position, pitchThrottleForce * rayLength);

						Gizmos.color = (currentRollThrottle > 0) ? Color.red : Color.green;
						Gizmos.DrawRay(droneRigidBody.transform.position, rollThrottleForce * rayLength);

						Gizmos.color = (currentYawThrottle > 0) ? Color.red : Color.green;
						Gizmos.DrawRay(droneRigidBody.transform.position, yawThrottleForce * rayLength);

						Gizmos.color = (currentThrottle > 0) ? Color.red : Color.green;
						Gizmos.DrawRay(droneRigidBody.transform.position, throttleForce * rayLength);
						...
						for(int i = 0; i < proppelerForces.Length; i++)
						{
							...
							Gizmos.DrawRay(proppelers[i].position + (droneRigidBody.transform.forward * 0.01f) + (proppelerForces[i] * rayLength), droneRigidBody.transform.up * proppelerForceBasedOnJoystickInput[i] * rayLength);
						}
					}			
			...
			public void DragManager()
			{
				...
				currentDrag = Mathf.Clamp(currentDrag, minDrag, maxDrag);
				*/
				droneRigidBody.drag = currentDrag; //calculated in a thread
			}			
			...
			{
				velocity = droneRigidBody.velocity.magnitude * 3.6f;
				angularVelocity = droneRigidBody.angularVelocity.magnitude;
			}
			...
			{
				droneRigidBody.transform.eulerAngles = new Vector3(droneRigidBody.transform.eulerAngles.x, droneRigidBody.transform.eulerAngles.y +180, 0);
			}
			...
			public void MovementUpDown()
			{
				if (Vertical_I >= 0)
				{
					...
					throttleForce = currentThrottle * maxThrottleForce * droneRigidBody.transform.up;
				}

				OurDroneTransformUp = droneRigidBody.transform.up;
				...
				for (int i = 0; i < proppelers.Length; i++)
				{
					...
					droneRigidBody.AddForceAtPosition(proppelerForces[i], proppelers[i].transform.position, ForceMode.Force);
				}
			}
			...
			public void PitchingRollingYawing()
            {
				...
				ourDroneTransformForward = droneRigidBody.transform.forward;
				ourDroneTransformRight = droneRigidBody.transform.right;
				ourDroneTransformUp = droneRigidBody.transform.up;
				ourDroneRotation = droneRigidBody.transform.rotation.eulerAngles;


				if (rollForce.magnitude > 0f)
				{
					droneRigidBody.angularDrag = maxAngularDrag;
				}
				else
				{
					droneRigidBody.angularDrag = Mathf.Lerp(droneRigidBody.angularDrag, minAngularDrag, Time.deltaTime * angularDragZeroingTime);
				}

				if (flightRecorderOverride) return; //if we are not playbacking our flight then use physics as per usual...

				droneRigidBody.angularVelocity = Vector3.Lerp(droneRigidBody.angularVelocity, Vector3.zero, Time.deltaTime * rotationSlowDown);
				droneRigidBody.angularVelocity += rollForce;				
			}
